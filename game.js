(() => {
  ...
  function doChainReaction(baseValue, level = 1, maxLevel = 2) {
    ...
    for (let i = 0; i < children; i++) {
      ...
      for (let p = 0; p < 18; p++) particles.push(new Particle(Math.random() * canvas.width, Math.random() * (canvas.height / 2), { color: `hsl(${Math.random() * 360},100%,60%)` }));
    }
  }
  ...
  function computeAndAward(baseMoney, isChild = false, chainLevel = 1) {
    ...
    if(goldenMultiplier>1) flashScreen("#ffd700",180); else if(luckyMultiplier>1) flashScreen("#00ff00",140);
  }

  function flashScreen(color,duration=150){ctx.fillStyle=color;ctx.fillRect(0,0,canvas.width,canvas.height);}
  ...
  function animate(){
    ...
    else{ctx.shadowBlur=0;ctx.shadowColor="#000";}
  ...
  function bindOwnerButtons(){if(!ownerCommands)return;const buttons=ownerCommands.querySelectorAll("button");buttons.forEach(btn=>{const evtName=(btn.dataset&&btn.dataset.event)?btn.dataset.event:btn.id||btn.getAttribute("data-event");const idMap={"doubleMoney":"doubleMoney","toggleDouble":"doubleMoney","rainbowMode":"rainbowMode","toggleRainbow":"rainbowMode","chaosRoll":"chaosRoll","toggleChaos":"chaosRoll","blackout":"blackout","toggleBlackout":"blackout","glitch":"glitch","toggleGlitch":"glitch","chainReaction":"chainReaction","toggleChain":"chainReaction","cosmicMode":"cosmicMode","toggleCosmic":"cosmicMode","luckyExplosions":"luckyExplosions","toggleLucky":"luckyExplosions","goldenFirework":"goldenFirework","triggerAdminStorm":"adminStorm","adminStorm":"adminStorm"};const eventKey=idMap[evtName]||evtName;btn.addEventListener("click",()=>triggerEventByName(eventKey));});}

  function clearEventTimers(name){if(eventIntervals[name]){clearTimeout(eventIntervals[name]);delete eventIntervals[name];}}

  function triggerEventByName(name,desc,duration=8000){clearEventTimers(name);EVENTS_STATE[name]=true;showEventBanner(name,desc,Date.now()+duration);eventIntervals[name]=setTimeout(()=>{EVENTS_STATE[name]=false;hideEventBanner();},duration);}

  auth.onAuthStateChanged(async user=>{
    if(user){currentUser=user.uid;hide(loginBox);show(ui);show(ownerCommands);await loadProgressOnline();bindOwnerButtons();animate();}
